...
`megaStav 0/1/2` a `aktualnyUnoStav() 0/1/2` zostávajú kompatibilnou health/agreement telemetriou, nie druhou autoritou `SYSTEM_MODE`. Mega posiela `0`, ak sú výsledné pool/T2/T3 hodnoty platné z primárnych zdrojov a `systemDegradovany=false`; `1`, ak sú všetky tri výsledné hodnoty platné, ale používa sa schválený fallback alebo existuje iná nekritická degradácia; `2` iba vtedy, ak chýba platná výsledná pool, T2 alebo T3 hodnota a Mega/control vrstva už nie je dôveryhodná pre SMART agreement. Uno povoľuje stabilizáciu pri `megaStav` 0 aj 1 a odmieta ju pri 2. Serial Mega zobrazuje `SYSTEM_MODE=... REASON=...` v 10 s diagnostike a okamžitý event iba pri zmene režimu.
...
LCD 20×4 na spoločnej I²C zbernici používa štyri neblokujúco rotované obrazovky: `PREVÁDZKA`, `PROSTREDIE`, `MANUÁLNE REŽIMY` a `PORUCHY / DIAGNOSTIKA`. Bežný interval každej stránky je 5 s. Ak existuje aktuálny problém z rovnakého spoločného zoznamu podmienok, ktorý vytvára USB Serial súhrn `PROBLEM:`, stránka porúch sa pri svojom príchode drží 30 s a potom sa rotácia vždy vráti na prvú stránku. Pri `PROBLEM: NONE` sa drží iba 5 s. Agreement vypnutý počas normálneho priebehu `SMART_STABLE=n/180s` nie je problém a 30-sekundové držanie nevyvolá. Obrazovka nastavovania teploty má naďalej nadradenú prioritu. Rotácia používa `millis()`, bez `delay()`, `String` a samostatnej paralelnej diagnostickej logiky.
...
Stav Mega v bajte 11 má význam: `0 = MEGA_HEALTH_OK`, teda všetky potrebné výsledné pool/T2/T3 hodnoty sú platné bez degradácie; `1 = MEGA_DEGRADED_TRUSTED`, teda všetky tri výsledné hodnoty sú platné cez primárny zdroj alebo schválený fallback a Mega zostáva dôveryhodná pre agreement; `2 = MEGA_CONTROL_CRITICAL`, teda najmenej jedna potrebná výsledná pool/T2/T3 hodnota nemá platný zdroj. Platný T4 alebo UNO_T1 pool fallback, UNO_T2 fallback a UNO_T3 fallback preto dávajú `megaStav=1`, nie 2. Tento bajt je iba kompatibilný vstup health/agreement pre Uno; jedinou autoritou runtime režimu zostáva `SystemMode systemMode` na Mega.
...
- Implementovaný SMART fallback na Mega sa aktivuje iba pri lokálnej neplatnosti a používa poradie MEGA_T1 → MEGA_T4 → čerstvý UNO_T1. Platný nesúlad vytvorí iba diagnostický konflikt. Finálne využitie výsledku v BASIC, potvrdenie konfliktu, hysterézia, recovery a safety následky zostávajú `NÁVRH – TREBA FYZICKY OVERIŤ`.
...
- Implementovaný SMART fallback na Mega je MEGA_T2 → čerstvý UNO_T2. Platný nesúlad vytvorí diagnostický konflikt bez automatického prevzatia. Finálne využitie výsledku v BASIC, potvrdenie konfliktu, hysterézia, recovery a safety následky zostávajú `NÁVRH – TREBA FYZICKY OVERIŤ`.
...
- V SMART režime zostáva `MEGA_T3` primárny. Ak je lokálny MEGA_T3 neplatný a vzdialený UNO_T3 je platný a čerstvý, Mega použije `UNO_T3_REMOTE_FALLBACK`. Ak nie je platný ani jeden, výsledná T3 je `INVALID`. Pri dvoch platných hodnotách zostáva bez porovnávania víťazom MEGA_T3.
...
Implementácia nemení fyzickú mapu: `MEGA_R9 / D22 = filtrácia`, `MEGA_R10 / D23 = solár/chrlič`. Budúci flow senzor môže potvrdiť skutočný chod, ale nie je súčasťou tohto checkpointu.
