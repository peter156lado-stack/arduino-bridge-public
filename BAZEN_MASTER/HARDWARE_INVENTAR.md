...
| XKC-Y25-NPN 5–12 V | 1 | 0 | plánovaný spoločný vstup Mega/Uno | nadradený LOW WATER FIL/SOLAR | 1 | MÁME / NEOTESTOVANÉ |
...
| H/L relé 1 kanál 5 V – spoločná evidencia oboch objednávok | 5 | 5 | Uno agreement D9; Mega agreement D31; Uno TOTAL STOP A0; Mega TOTAL STOP D32; W1209 fackovač D33 | agreement, dve nezávislé TOTAL STOP autority a power-cycle 12 V W1209 | 0 | VŠETKÝCH 5 FYZICKY PRIPOJENÝCH/PRIDELENÝCH; `HIGH→COM–NO`, `LOW/strata napájania→COM–NC` FYZICKY OVERENÉ; automatické TOTAL STOP/W1209 fault podmienky NEIMPLEMENTOVANÉ |
| PC817 izolačný modul HY-M154, 4 kanály | 2 | 1 modul fyzicky použitý, 2 kanály; druhý modul plánovaný pre XKC | 2. modul: dva samostatné fyzicky overené UART kanály Mega→Uno a Uno→Mega, dva kanály voľné; 1. modul zostáva pre budúce dve XKC read paths | UART galvanické oddelenie s oddelenými GND a lokálnymi 4,7 kΩ pull-upmi; odstránenie UART phantom/backfeed; prvý modul budúci XKC | na UART module 2 voľné kanály; XKC modul zatiaľ nepoužitý | MÁME / UART PHYSICALLY VERIFIED–PASS / XKC PLÁNOVANÉ |
...
Fyzické kanály 1–4 jedinej 16R dosky patria výhradne BASIC_R1–R4; pôvodná väzba týchto kanálov na Mega D30–D33 bola odstránená zo zdrojového kódu aj fyzickej kabeláže. D32/D33 boli neskôr nanovo pridelené samostatným modulom MEGA_TOTAL_STOP/W1209_FACKOVAC, nejde o obnovenie väzby na 16R kanály 3/4. BASIC_R1 a BASIC_R2 sú fyzicky v motorových cestách za spoločným TOTAL STOP/WAGO rozdelením. BASIC_R3 fyzicky pokračuje cez `COM–NC` v 12 V napájacej ceste W1209; BASIC_R4 zostáva rezervou. Konkrétne Uno/BASIC riadiace piny nie sú potvrdené. Kanály 5–16 zostávajú Mega/SMART; aktívne sú `MEGA_R9/D22 = filtrácia` a `MEGA_R10/D23 = solár/chrlič`. Pri mŕtvej Mega bol fyzicky potvrdený residual backfeed z napájanej 16R dosky približne 1,58 V a reálne zopnutie R5–R16; Mega pritom funkčne nežije a účinok SMART kanálov je blokovaný nadradenou agreement/BASIC povoľovacou cestou. Stav je podľa MASTER `KNOWN / PHYSICALLY CONFIRMED / FUNCTIONALLY ISOLATED / ACCEPTED`. D31 je pripojený na Mega agreement modul a D9 na Uno agreement modul; polarita agreement kusov je fyzicky overená ako `HIGH → COM–NO`, `LOW/strata napájania → COM–NC`.
...
Jeden MicroSD modul s kartou 4 GB FAT32 je fyzicky zapojený na Uno a otestovaný. UNO zapisuje `UNO_LOG.CSV` v intervale 60 s a zmeny lokálnych/komunikačných stavov do `UNO_EVT.CSV`; chyba SD nesmie ovplyvniť meranie ani ostatné funkcie a obnova SD sa skúša automaticky raz za 60 s. Druhý MicroSD modul a 1 GB microSD karta zatiaľ nie sú fyzicky otestované v module. RTC DS3231 ostáva iba na Mega. Diagnostická linka Mega Serial2 D16/D17 ↔ Uno D7/D8 pri 9600 Bd, MASTER→REPLY a V4 protokol 22/24 B boli fyzicky overené 22. 8. 2026. Linka nie je safety prvok.
